...
Install the Python package persistently first so the Pi process can resolve `obsidian-vault-mcp` on `PATH`. Use the package selector after 3.0.0 is published, or replace it with `"<WHEEL_PATH>"` for local production acceptance:

```bash
pipx install "zotero-obsidian-mcp==3.0.0"
# or: uv tool install "zotero-obsidian-mcp==3.0.0"
```
