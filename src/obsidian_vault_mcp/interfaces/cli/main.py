...
def build_parser() -> argparse.ArgumentParser:
    parser = _Parser(prog="obsidian-vault-mcp", description="Zotero + MinerU + Obsidian literature workflow")
...
def _tool_arguments(namespace: argparse.Namespace) -> dict[str, Any]:
    ...
    return {key: value for key, value in vars(namespace).items() if key not in excluded}
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the production CLI and emit one machine-readable JSON value."""
