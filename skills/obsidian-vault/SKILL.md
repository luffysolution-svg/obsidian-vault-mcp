...
When the document being parsed belongs to a Zotero item already ingested into the vault, pass `zotero_key=<key>` to `obsidian_mineru_extract_and_ingest`. On success the tool will locate the corresponding literature note in the vault and append a `mineru_markdown` wikilink field to its YAML frontmatter, leaving all other fields and body content unchanged. The recommended three-step workflow is:
1. `obsidian_ingest_zotero_item` → creates the literature note in `literature/` with full YAML, notes, and annotations.
2. `obsidian_mineru_extract_and_ingest` with `zotero_key` → parses the PDF into a separate MinerU source note and adds `mineru_markdown: [[...]]` to the literature note YAML.
3. The literature note now links directly to the full parsed text; no body content is modified.
...
Use `obsidian_zotero_ping` before direct Zotero work. If it fails, ask the user to open Zotero Desktop and enable local API access. Once reachable, use `obsidian_zotero_search_items`, `obsidian_zotero_get_item`, `obsidian_zotero_get_children`, and `obsidian_ingest_zotero_item`. Imported Zotero notes include `zoteroKey`, `zoteroSelect`, `zoteroLinks`, and PDF links when PDF children exist. Attachment copy naming can use `original`, `zotero_key`, `citekey`, `title_year`, or `parent_key`. When a Zotero item is re-ingested and its `zoteroVersion` is unchanged the tool returns `upToDate: true` and skips writing; when the version has changed the note is fully overwritten.
